...
export default function Navbar() {
  ...
  const fetchNotifications = () => {
    ...
    api.get('/notifications').then(r => {
      ...
      setUnreadNotifications(r.data.unread_count || 0)
    }).catch(() => {})
  }
  ...
  const markUnread = async (id) => {
    await api.put(`/notifications/${id}/unread`).catch(() => {})
    setNotifications(prev => prev.map(n => n.id === id ? { ...n, is_read: false } : n))
    setUnreadNotifications(unread_notifications + 1)
  }
                  ...
                  {bellOpen && (
                    <motion.div
                      ...
                      <div className="flex items-center justify-between px-4 py-3 border-b border-dark-700">
                        ...
                        {unread_notifications > 0 && (
                          <button onClick={markAllRead} className="flex items-center gap-1 text-xs text-brand-400 hover:text-brand-300 transition-colors">
